...
y_metric <- "padj"              # Use FDR-adjusted p-values for conservative visualization
fc_cutoff <- 0.485              # 1.4x fold-change (more sensitive)
...
count_filter <- "10c2s"  # 10 counts in 2+ samples
filter_suffix <- sprintf("manual_filter_%s_bm%d_fc%.1fx_p%.2f_%s", count_filter, base_mean_min, fc_linear, 0.05, y_metric)
...
volcano_plot <- EnhancedVolcano::EnhancedVolcano(
  ...
  pointSize = 2.0,
  labSize = 3.0,
  title = "Disease vs Healthy Control (DESeq2 - Manual Filtering - FDR Y-axis)",
