...
filter_suffix <- sprintf("no_filter_fc%.1fx_p%.2f_%s", fc_linear, 0.05, y_metric)
...
# NO FILTERING AT ALL - use all genes regardless of expression level
# This includes genes with zero counts across all samples
...
# independentFiltering=FALSE - no filtering at all, keep every single gene
res_unshrunk <- as.data.frame(DESeq2::results(dds, name = coef_name, independentFiltering = FALSE))
...
# Note: NO FILTERING AT ALL - even DESeq2's independent filtering is disabled
# All genes will have results, including those with zero counts
...
volcano_plot <- EnhancedVolcano::EnhancedVolcano(
  ...
  pointSize = 2.0,
  labSize = 3.0,
  title = "Disease vs Healthy Control (DESeq2 - No Filtering)",
